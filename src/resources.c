                ...
                if((buffer = malloc(length + 1))) {
                    if(PHYSFS_readBytes(file, buffer, length) == length) {
                        ...
                        *(buffer + length) = 0;
                        ...
                        ir_error("ir_resources_fetch_lua: Failed to read file \"%s\": %s", name, PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode()));
                    }
                    free(buffer);
                } else {
                    ir_error("ir_resources_fetch_lua: Failed to allocate buffer for \"%s\"", name);
                }
                PHYSFS_close(file);
            } else {
                ir_error("ir_resources_fetch_lua: Failed to open \"%s\": %s", name, PHYSFS_getErrorByCode(PHYSFS_getLastErrorCode()));
            }
        } else {
