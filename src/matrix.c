...
// NOTE: For potential windows support we'd need _aligned_malloc instead
// ~FabricatorZayac
...
int ir_matrix_from_lua(lua_State *L) {
	...
	for (int i = 0; i < 16; i++) {
		...
		if (!lua_isnumber(L, -1)) {
			typename = lua_typename(L, lua_type(L, -1));
			lua_pop(L, 2);
			return ir_push_error_lua(
				L, "Expected number, provided %s", typename
			);
		}
...
int ir_matrix_index_lua(lua_State *L) {
	...
	// TODO: Include the ability to directly manipulate the matrix from Lua
	// ~ahill
	if (lua_isstring(L, -1)) {
